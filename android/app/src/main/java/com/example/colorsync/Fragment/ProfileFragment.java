...
public class ProfileFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        adapter = new ImageGridAdapter(items, 40, own);
        ...
        settings.setOnClickListener(v -> MainActivity.getInstance().goToSettings());

        uploadOpenButton.setOnClickListener(v -> uploadOpen(false));

        uploadClose.setOnClickListener(v -> uploadOpen(false));
    ...
    private void upload() {
        ...
        if (isPfp) {
            ...
            MainActivity.getApi().uploadImage(UserManager.getBearer(), fileBody, visibilityRequest).enqueue(new Callback<IdType>() {
                @Override
                public void onResponse(@NonNull Call<IdType> call, @NonNull Response<IdType> response) {
                    if (response.isSuccessful() && response.body() != null) {
                ...
                @Override
                public void onFailure(@NonNull Call<IdType> call, @NonNull Throwable throwable) {
